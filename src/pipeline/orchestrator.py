"""
Orchestrateur principal de la pyramide d'analyse territoriale (Stages 1 à 4).

Permet d'exécuter soit la chaîne complète (Stage 1 -> 2 -> 3 -> 4),
soit un stage unitaire avec reprise automatique sur incident.
"""
import time
...
def run_pipeline(stage: str = "all", run_id: Optional[str] = None, force: bool = False) -> Dict:
    ...
    try:
        ...
        logger.error(f"❌ Arrêt prématuré du pipeline sur erreur : {exc}")
        raise
    finally:
